defmodule ClippexWeb.UserAuthTest do
  use ClippexWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Clippex.Accounts
  alias Clippex.Accounts.Scope
  alias ClippexWeb.UserAuth

  import Clippex.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ClippexWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ClippexWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ClippexWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
